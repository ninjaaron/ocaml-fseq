...
(** A module for for non-empty sequences. Only those operations which differ
    from the {!Fseq.t} are documented. Operations which reduce the size of the
    sequence are not included, as there is no way to guarantee that reducing
    operations will create a non-empty sequence. *)
module NonEmpty : sig
  type 'a t
  ...
  (** Unfold a sequence from an initial state. The initial state is passed to
      [f]. [f] produces either [None], in which case the the sequence is
      finished, or [Some] of a pair of an element and the next state to passed
      to [f]. Function returns [None] if the [f init] is [None] else [Some]
      non-empty sequence *)
  ...
  val lview : 'a t -> 'a * 'a t option
  (** "Uncons" on the left side of the sequence. There is always a head, in a
      non-empty sequence, but the tail is an option in case the sequence is
      exhausted *)
  ...
  (** Uncons from the right side of the sequence (or "unsnoc", if you like).
      there is always a head, but the tail is an option in case the sequence is
      exhausted. *)
  ...
  (** Remomve the left-most element and return the rest of the sequence. Raises
      an exception with a single-element sequence. *)

  val tl_right_exn : 'a t -> 'a t
  (** Remomve the right-most element and return the rest of the sequence. Raises
      an exception with a single-element sequence. *)
...
val to_nonempty : 'a t -> 'a NonEmpty.t option
val of_nonempty : 'a NonEmpty.t -> 'a t
